...
class GameOfLife:
    ...
    def get_neighbourslen(self, cell: Cell) -> int:

        return sum(self.get_neighbours(cell))

    def get_next_generation(self) -> Grid:
        new_grid = self.create_grid(False)
        for i in range(self.cols):
            for j in range(self.rows):
                if (self.curr_generation[i][j] == 0) and (self.get_neighbourslen([i, j]) == 3):
                    new_grid[i][j] = 1
                elif (self.curr_generation[i][j] == 1) and (2 <= self.get_neighbourslen([i, j]) <= 3):
                    new_grid[i][j] = 1
